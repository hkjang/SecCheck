...
관리자 설정 > 메일에서 `테스트 메일 받는 주소`를 적고 `테스트 메일 보내기`로 SMTP 경로를 먼저 확인할 수 있습니다. 같은 탭의 `서비스 주소`를 채우면 알림 메일에 해당 심의로 바로 가는 링크가 포함됩니다. 비워 두면 링크 없이 발송합니다.

사용자는 알림 화면의 `수신 설정`에서 유형별로 이메일을 끄거나, 즉시 수신 대신 하루 한 번 요약으로 바꿀 수 있습니다. 인앱 알림은 설정과 무관하게 항상 기록되므로 알림 이력이 사라지지 않습니다. 요약은 `mail.digest_hour`(기본 8시) 이후 사용자당 하루 한 번 발송됩니다. 요약에 실제로 담긴 알림만 발송 완료로 표시하므로, 발송 중에 새로 도착한 알림은 다음 요약에 포함됩니다. 발송에 실패하면 아무것도 표시하지 않아 다음 주기에 다시 시도합니다.
