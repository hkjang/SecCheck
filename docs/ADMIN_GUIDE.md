...
`테스트 메일 보내기` 는 저장된 설정으로 `테스트 메일 받는 주소` 에 적은 주소에 1통을 보내고 결과를 그 자리에서, 그리고 아래 `메일 발송 기록` 에 남깁니다. 주소 칸은 본인 프로필의 이메일로 채워져 있지만 바꿔서 보낼 수 있으므로, 프로필에 이메일이 없는 관리자도 확인할 수 있고 릴레이가 특정 도메인만 받는지도 그 자리에서 알 수 있습니다. 릴레이 설정은 한 번에 맞는 일이 드물므로 운영 전에 반드시 한 번 확인하십시오.
