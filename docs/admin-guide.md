...
- **배포 자체 점검**: `/app/seccheck selftest`로 준비 상태, 로그인, OpenAPI, 게시 템플릿, 감사 체인을 확인합니다. `--full`은 검증 환경에서 심의 생성과 Excel·PDF·ZIP 내보내기까지 수행하며, 한글 폰트 누락 같은 이미지 패키징 결함은 이 단계에서만 드러납니다. 실패 시 종료 코드 1.
