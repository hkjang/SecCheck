...
- **대기열 상태**(보안 담당자·감사): 담당자가 없는 심의와 3일 이상 멈춘 심의 수를 보여 주고 같은 조건의 목록으로 연결합니다. 팀이 조치를 마쳐 **확인만 남은 보완 요청** 건수도 함께 표시합니다(기한과 무관하게, 확인해 줘야 심의가 끝나는 일이기 때문입니다). 정체 판정 기준(기간과 상태)은 `심의 정체` 알림과 한 곳에서 공유하므로 화면의 숫자와 메일이 어긋나지 않습니다.
...
- **MCP SSO**: 개인 키 체계는 그대로 두고 Keycloak 액세스 토큰으로도 `/mcp` 에 들어오게 합니다(OAuth 2.1 리소스 서버 — 메타데이터 문서·401 안내·서명/발급자/대상 검사). MCP 클라이언트에 `MCP 주소` 하나만 주면 스스로 로그인해 토큰을 받아 오며, 웹으로 로그인한 활성 계정만 통과하고 범위는 `SSO 토큰 범위` 가 정합니다. 기본은 꺼짐
