...
- **대기열 상태**(보안 담당자·관리자): 담당자가 없는 심의와 3일 이상 멈춘 심의 수를 보여 주고 같은 조건의 목록으로 연결합니다. 정체 판정 기준(기간과 상태)은 `심의 정체` 알림과 한 곳에서 공유하므로 화면의 숫자와 메일이 어긋나지 않습니다.
...
- **이전 심의 증적 가져오기**: 같은 서비스의 지난 심의에 첨부된 증적을 항목 단위로 이 심의에 복사. 파일은 원본 키로 복호화해 가져오는 사람의 키로 다시 암호화하며(증적 1건 = 키 1개 원칙 유지), 해시를 비교해 동일한 파일인지 확인합니다. 검사를 통과하지 않은 파일은 제외
