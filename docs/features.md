...
- **판정 이후 답변이 바뀐 항목 감지**: 보완 요청은 심의 전체를 작성자에게 돌려보내므로 요청하지 않은 항목까지 수정될 수 있습니다. 판정보다 답변이 나중에 저장된 항목은 `검토 후 답변 변경` 뱃지로 표시하고, 남아 있는 동안에는 검토 완료를 거부하며 몇 건이 남았는지 알려 줍니다.
