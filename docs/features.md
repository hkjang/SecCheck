...
- **접근성**: 대화 상자는 열릴 때 포커스를 가져가고 Tab이 그 안에서만 순환하며 닫으면 원래 위치로 포커스를 돌려줍니다(`role="dialog"`, `aria-modal`). 모든 입력 항목의 라벨이 해당 컨트롤에 연결되어 있어 화면 낭독기가 이름을 읽고 라벨 클릭으로 포커스가 이동합니다. 도움말과 오류 문구도 컨트롤에 연결되어 함께 읽힙니다
