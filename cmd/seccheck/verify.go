...
func verifyEvidence(args []string) int {
	...
	query := `SELECT e.id,e.original_filename,e.stored_filename,e.key_owner_id,e.key_version,e.current_version,e.size_bytes,e.sha256 FROM evidences e WHERE e.deleted_at IS NULL ORDER BY e.created_at`
	if sample > 0 {
		query = `SELECT e.id,e.original_filename,e.stored_filename,e.key_owner_id,e.key_version,e.current_version,e.size_bytes,e.sha256 FROM evidences e WHERE e.deleted_at IS NULL ORDER BY random() LIMIT ` + fmt.Sprint(sample)
	}
