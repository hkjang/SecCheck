...
func (w *Worker) Sweep(ctx context.Context) map[string]int64 {
	...
	removed := map[string]int64{
		...
		"mail_deliveries":  w.delete(ctx, `DELETE FROM mail_deliveries WHERE ctid IN (SELECT ctid FROM mail_deliveries WHERE created_at<now()-make_interval(days=>$2) LIMIT $1)`, retention),
	}
...
func (w *Worker) alertStalledQueue(ctx context.Context) int64 {
	...
	body := fmt.Sprintf("실행 시각이 지난 작업이 %d분째 대기 중입니다. 이메일 알림과 증적 악성코드 검사가 멈춘 상태일 수 있으니 서버 로그에서 notification·scanner 구성요소의 오류를 확인하세요.", int64(waited)/60)
	var sent int64
	for _, admin := range admins {
