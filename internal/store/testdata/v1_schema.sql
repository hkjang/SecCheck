-- The schema exactly as the first release shipped it, kept as a fixture.
-- An installation created from this file recorded schema_migrations version 1,
-- so every later release skips 001_baseline.sql for it: whatever the baseline
-- gained afterwards can only reach that database through a numbered migration.
-- TestUpgradingTheFirstReleaseReachesTheSameSchema upgrades a copy of this and
-- compares it with a fresh install, so the next edit to the baseline that is
-- not repeated in a numbered file fails the build. Never edit this file.
