...
import { Activity, Bell, BookOpen, Boxes, CheckSquare, ChevronDown, ClipboardCheck, FileKey, Gauge, History, KeyRound, LayoutDashboard, ListChecks, Logs, Search, Settings, Shield, ShieldCheck, Sparkles, Users, X } from 'lucide-react'
...
const navigation: Nav[] = [
  ...
  { to: '/profile/keys', label: '개인 키 관리', icon: FileKey, section: '개인화' },
  { to: '/integrations', label: 'API · MCP', icon: Sparkles },
]

const titles: Record<string, [string, string]> = {
  '/': ['대시보드', 'SecCheck 보안성 심의 현황'], '/reviews': ['심의 관리', '작성·제출·보완 상태를 관리합니다'], '/reviews/new': ['신규 심의', '서비스 정보를 기준으로 체크리스트가 자동 배정됩니다'], '/security': ['보안 검토 Queue', '신규 제출과 보완 건을 검토합니다'], '/controls': ['Security Controls', '중복 통제를 통합하고 영향 범위를 추적합니다'], '/templates': ['템플릿 관리', '체크리스트 원본과 게시 버전을 관리합니다'], '/templates/import': ['Excel 가져오기', '기존 체크리스트를 검증 후 템플릿으로 전환합니다'], '/admin/users': ['사용자·역할', 'RBAC 사용자와 최소 권한을 관리합니다'], '/admin/settings': ['서비스 관리자 설정', '운영 설정과 OIDC를 중앙 관리합니다'], '/admin/audit': ['감사로그', '중요 행위의 해시 체인 이력을 확인합니다'], '/admin/logs': ['서버 로그', '구조화된 애플리케이션 로그를 확인합니다'], '/profile/keys': ['개인 키 관리', '개인 API 키와 암호화 키를 회전합니다'], '/integrations': ['API · MCP', '보안 자동화와 AI 에이전트 연계를 구성합니다']
}
...
export default function Layout() {
  ...
  return <div className="app-shell">
    ...
    <main className="content-shell">
      <header className="topbar"><div className="topbar-title"><strong>{pathTitle[0]}</strong><span>{pathTitle[1]}</span></div><div className="top-actions"><Link to="/admin/logs" className="icon-button" aria-label="운영 상태"><Activity size={18} /></Link><Link to="/notifications" className="icon-button" aria-label="알림"><Bell size={18} /></Link><button className="profile-button" onClick={() => setProfile(v => !v)}><div className="avatar">{user.display_name.slice(0, 1)}</div><div className="profile-meta"><strong>{user.display_name}</strong><span>{user.department || user.username}</span></div><ChevronDown size={15} /></button>{profile && <div className="profile-menu"><Link to="/profile" onClick={() => setProfile(false)}><Users size={16} /> 프로필</Link><Link to="/profile/keys" onClick={() => setProfile(false)}><KeyRound size={16} /> 개인 키 관리</Link><button onClick={logout}><X size={16} /> 로그아웃</button><div className="profile-version">SecCheck v{version}</div></div>}</div></header>
      <Outlet />
    </main>
    {command && <Modal onClose={() => { setCommand(false); setQuery(''); setResults({}) }} className="command-modal"><div style={{ margin: -20 }}><Search style={{ position: 'absolute', margin: '18px' }} size={20} /><input ref={inputRef} className="command-input" placeholder="메뉴, 심의번호, 서비스명, 보안항목, 증적 검색…" value={query} onChange={e => setQuery(e.target.value)} /><div className="command-list">{!query && visible.map(item => { const Icon = item.icon; return <button className="command-item" key={item.to} onClick={() => { navigate(item.to); setCommand(false) }}><Icon size={18} /><span>{item.label}</span></button> })}{results.reviews?.map(x => <button className="command-item" key={x.id} onClick={() => { navigate(`/reviews/${x.id}`); setCommand(false) }}><ClipboardCheck size={18} /><span><strong>{x.review_number}</strong> {x.service_name}</span></button>)}{results.items?.map((x, i) => <button className="command-item" key={`${x.review_id}-${i}`} onClick={() => { navigate(`/reviews/${x.review_id}`); setCommand(false) }}><BookOpen size={18} /><span><strong>{x.item_code}</strong> {x.title}</span></button>)}{results.evidences?.map(x=><button className="command-item" key={x.id} onClick={()=>{navigate(`/reviews/${x.review_id}`);setCommand(false)}}><FileKey size={18}/><span><strong>{x.review_number}</strong> {x.original_filename}</span></button>)}</div></div></Modal>}
  </div>
}
