...
export default function UsersPage() {
  ...
  const load = () => { const qs = new URLSearchParams({ limit: String(limit) }); if (query.trim()) qs.set('q', query.trim()); if (only) qs.set('only', only); return get<{ items: User[]; total: number; locked: number; has_more: boolean }>(`/api/v1/admin/users?${qs}`).then(out => { setUsers(out.items); setPage({ total: out.total, locked: out.locked, has_more: out.has_more }) }) }
  useEffect(() => { const timer = window.setTimeout(() => { load().catch(() => undefined) }, 250); return () => window.clearTimeout(timer) }, [query, only, limit])
