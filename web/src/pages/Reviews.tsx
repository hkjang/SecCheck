...
const emptyFilter = { q: '', status: '', result: '', department: '', reviewer_id: '', from: '', to: '', overdue: '', open_at_risk: '', unassigned: '', stalled: '', mine: '' }
const sorts: [string, string][] = [['updated', '최근 변경순'], ['created', '생성일순'], ['open_date', '오픈 예정일순'], ['number', '심의번호순'], ['service', '서비스명순'], ['status', '상태순']]


// 승인 하나로 뭉뚱그리면 "깨끗하게 통과"와 "할 일 세 개를 남기고 통과"가 같아 보입니다.
const listResultLabel: Record<string, string> = { APPROVED: '승인', CONDITIONAL: '조건부', REJECTED: '반려' }
  ...
  return <div className="page">
    ...
    <div className="card"><div className="card-body">
      <div className="toolbar"><div className="search-box"><Search /><input className="input" aria-label="심의 검색" placeholder="심의번호, 서비스명, 부서 검색" value={filter.q} onChange={e => set('q', e.target.value)} /></div>
        ...
        <button type="button" className={`chip ${filter.result === 'CONDITIONAL' ? 'on' : ''}`} aria-pressed={filter.result === 'CONDITIONAL'} onClick={() => set('result', filter.result === 'CONDITIONAL' ? '' : 'CONDITIONAL')}>조건부 승인</button>
        ...
        <button type="button" className={`chip ${filter.overdue ? 'on' : ''}`} aria-pressed={Boolean(filter.overdue)} onClick={() => set('overdue', filter.overdue ? '' : '1')}><AlertTriangle size={13} /> 기한 초과</button><button type="button" className={`chip ${filter.unassigned ? 'on' : ''}`} aria-pressed={Boolean(filter.unassigned)} onClick={() => set('unassigned', filter.unassigned ? '' : '1')}><UserRound size={13} /> 담당자 없음</button><button type="button" className={`chip ${filter.stalled ? 'on' : ''}`} aria-pressed={Boolean(filter.stalled)} onClick={() => set('stalled', filter.stalled ? '' : '1')}><Hourglass size={13} /> 정체</button><button type="button" className={`chip ${filter.open_at_risk ? 'on' : ''}`} aria-pressed={Boolean(filter.open_at_risk)} onClick={() => set('open_at_risk', filter.open_at_risk ? '' : '1')} title="오픈 예정일이 2주 이내인데 심의가 끝나지 않은 건"><CalendarClock size={13} /> 오픈 임박</button>
