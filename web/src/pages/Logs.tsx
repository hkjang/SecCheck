...
export default function LogsPage() {
  ...
  useEffect(() => {
    let alive = true
    const load = () => { const qs = new URLSearchParams({ limit: '200' }); if (level) qs.set('level', level); if (query.trim()) qs.set('q', query.trim()); return get<Record<string, unknown>[]>(`/api/v1/admin/logs?${qs}`).then(v => { if (alive) setItems(v) }) }
