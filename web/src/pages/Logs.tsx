...
import { Badge, Button, Empty, Loading, Toggle, formatDate } from '../components/ui'
...
export default function LogsPage() {
  ...
  useEffect(() => {
    let alive = true
    const load = () => { const qs = new URLSearchParams({ limit: '200' }); if (level) qs.set('level', level); if (query.trim()) qs.set('q', query.trim()); return get<{ items: Record<string, unknown>[]; has_more: boolean }>(`/api/v1/admin/logs?${qs}`).then(v => { if (alive) { setItems(v.items); setTruncated(Boolean(v.has_more)) } }) }
