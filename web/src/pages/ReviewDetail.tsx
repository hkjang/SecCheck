...
export default function ReviewDetail() {
  ...
  const load = async () => { const [r, i] = await Promise.all([get<Review>(`/api/v1/review-requests/${id}`), get<ChecklistItem[]>(`/api/v1/review-requests/${id}/items`)]); setReview(r); setItems(i); if (!selected && i[0]) setSelected(i[0].id); if (['DRAFT', 'CHANGE_REQUESTED'].includes(r.status)) { try { setPrecheck(await get<{ ready: boolean; issues: Record<string, unknown>[] }>(`/api/v1/review-requests/${id}/submission-check`)) } catch { setPrecheck(undefined) } } else setPrecheck(undefined) }
  ...
  const counts = useMemo(() => { const all = items || []; return { ALL: all.length, MISSING: all.filter(x => !x.response.applicability).length, NA: all.filter(x => x.response.applicability === 'N/A').length, EVIDENCE: all.filter(x => x.evidence_required && !x.evidences.length).length, CHANGE: all.filter(x => x.change_requests.some(c => c.status !== 'VERIFIED')).length, MINE: all.filter(x => x.response.assigned_to === user.id).length, STALE: all.filter(answerChangedSinceReview).length, CARRIED: all.filter(carriedOver).length, BLOCKED: all.filter(x => blocked.has(x.id)).length } }, [items, user.id, blocked])
