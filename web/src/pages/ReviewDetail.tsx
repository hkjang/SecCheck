...
export default function ReviewDetail() {
  ...
  const [review, setReview] = useState<Review>(); const [items, setItems] = useState<ChecklistItem[]>(); const [selected, setSelected] = useState<string>(''); const [open, setOpen] = useState<Set<string>>(new Set()); const [query, setQuery] = useState(''); const [filter, setFilter] = useState(itemFilters.includes(filterRequest) ? filterRequest : 'ALL'); const [validation, setValidation] = useState<Record<string, unknown>[] | null>(null); const [dialog, setDialog] = useState<'complete' | 'approval' | 'reject' | 'reopen' | null>(null); const [ruleOpen, setRuleOpen] = useState(false); const [busy, setBusy] = useState(false); const [picked, setPicked] = useState<Set<string>>(new Set()); const [bulkOpen, setBulkOpen] = useState(false); const [historyOpen, setHistoryOpen] = useState(false); const [precheck, setPrecheck] = useState<{ ready: boolean; issues: Record<string, unknown>[] }>(); const [handover, setHandover] = useState(false)
...
function DecisionModal({ kind, busy, draft, suggested, onClose, onSubmit }: { kind: 'complete' | 'approval' | 'reject' | 'reopen'; busy: boolean; draft?: string; suggested?: string; onClose: () => void; onSubmit: (data: unknown) => void }) { const [comment, setComment] = useState(draft || ''); const [result, setResult] = useState(suggested || 'APPROVED'); const complete = kind === 'complete'; return <Modal title={complete ? '검토 완료' : kind === 'approval' ? '최종 승인' : '심의 반려'} onClose={onClose} footer={<><Button onClick={onClose}>취소</Button><Button variant={kind === 'reject' ? 'danger' : 'primary'} disabled={busy || (kind === 'reopen' && !comment.trim())} onClick={() => onSubmit(complete ? { final_opinion: comment, final_result: result } : { comment })}>{complete ? '검토 완료' : kind === 'approval' ? '승인' : '반려'}</Button></>}><div className="form-grid">{complete && <Field label="심의 결과"><select className="select" value={result} onChange={e => setResult(e.target.value)}><option value="APPROVED">승인</option><option value="CONDITIONAL">조건부 승인</option><option value="REJECTED">반려</option></select></Field>}<Field label={complete ? '최종 의견' : '의견'} className="span-2"><LongText value={comment} onChange={setComment} /></Field></div></Modal> }
