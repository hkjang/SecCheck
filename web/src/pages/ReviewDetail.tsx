...
const carriedOver = (item: ChecklistItem) => Boolean(item.response?.carried_at)
...
export default function ReviewDetail() {
  ...
  const counts = useMemo(() => { const all = items || []; return { ALL: all.length, MISSING: all.filter(x => !x.response.applicability).length, NA: all.filter(x => x.response.applicability === 'N/A').length, EVIDENCE: all.filter(x => x.evidence_required && !x.evidences.length).length, CHANGE: all.filter(x => x.change_requests.some(c => c.status !== 'VERIFIED')).length, MINE: all.filter(x => x.response.assigned_to === user.id).length, STALE: all.filter(answerChangedSinceReview).length, CARRIED: all.filter(carriedOver).length } }, [items, user.id])
...
function ReviewVerdict({ result, onSaved }: { result: Record<string, unknown>; onSaved: () => Promise<void> }) {
  ...
  {reporting && <Modal title="조치 완료 보고" onClose={() => setReporting(false)}
    ...
    <Field label="조치 내용" required><textarea className="textarea" value={note} onChange={e => setNote(e.target.value)} /></Field>
  </Modal>}
  </>
}
