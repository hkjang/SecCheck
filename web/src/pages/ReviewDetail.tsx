...
// 회수·재개 dialogs ask for a reason the server requires under that name;
// approval and rejection leave an optional comment.
const decisionLabels: Record<'complete' | 'approval' | 'reject' | 'reopen' | 'withdraw', { title: string; button: string; field: string }> = { complete: { title: '검토 완료', button: '검토 완료', field: '최종 의견' }, approval: { title: '최종 승인', button: '승인', field: '의견' }, reject: { title: '심의 반려', button: '반려', field: '의견' }, reopen: { title: '보완 재개', button: '다시 열기', field: '재개 사유' }, withdraw: { title: '결재 요청 회수', button: '회수', field: '회수 사유' } }

function DecisionModal({ kind, busy, draft, suggested, onClose, onSubmit }: { kind: 'complete' | 'approval' | 'reject' | 'reopen' | 'withdraw'; busy: boolean; draft?: string; suggested?: string; onClose: () => void; onSubmit: (data: unknown) => void }) { const [comment, setComment] = useState(draft || ''); const [result, setResult] = useState(suggested || 'APPROVED'); const complete = kind === 'complete'; const needsReason = kind === 'reopen' || kind === 'withdraw'; const labels = decisionLabels[kind]; return <Modal title={labels.title} onClose={onClose} footer={<><Button onClick={onClose}>취소</Button><Button variant={kind === 'reject' ? 'danger' : 'primary'} disabled={busy || (needsReason && !comment.trim())} onClick={() => onSubmit(complete ? { final_opinion: comment, final_result: result } : needsReason ? { reason: comment } : { comment })}>{labels.button}</Button></>}><div className="form-grid">{complete && <Field label="심의 결과"><select className="select" value={result} onChange={e => setResult(e.target.value)}><option value="APPROVED">승인</option><option value="CONDITIONAL">조건부 승인</option><option value="REJECTED">반려</option></select></Field>}<Field label={labels.field} className="span-2"><LongText value={comment} onChange={setComment} /></Field></div></Modal> }
