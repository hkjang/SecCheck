...
export default function ReviewDetail() {
  ...
  const counts = useMemo(() => { const all = items || []; return { ALL: all.length, MISSING: all.filter(x => !x.response.applicability).length, NA: all.filter(x => x.response.applicability === 'N/A').length, EVIDENCE: all.filter(x => x.evidence_required && !x.evidences.length).length, CHANGE: all.filter(x => x.change_requests.some(c => c.status !== 'VERIFIED')).length, MINE: all.filter(x => x.response.assigned_to === user.id).length, STALE: all.filter(answerChangedSinceReview).length } }, [items, user.id])
